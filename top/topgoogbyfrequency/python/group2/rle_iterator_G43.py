...
class RLEIterator:
    ...
    def next(self, n: int) -> int:
        ...
        while acc < n:
            if not self.arr:
                return -1
            times, num = self.arr.pop(), self.arr.pop()
            acc += times
        if acc > n:
            self.arr.append(num)
            self.arr.append(acc - n)
        return num


def main():
    rle_iterator = RLEIterator([3, 8, 0, 9, 2, 5])  # This maps to the sequence [8,8,8,5,5].
    print(rle_iterator.next(2))  # exhausts 2 terms of the sequence, returning 8. The remaining sequence
    # is now [8, 5, 5].
    print(rle_iterator.next(1))  # exhausts 1 term of the sequence, returning 8. The remaining sequence is now [5, 5].
    print(rle_iterator.next(1))  # exhausts 1 term of the sequence, returning 5. The remaining sequence is now [5].
    print(rle_iterator.next(2))  # exhausts 2 terms, returning -1. This is because the first term exhausted was 5,
    # but the second term did not exist. Since the last term exhausted does not exist, we return -1.
