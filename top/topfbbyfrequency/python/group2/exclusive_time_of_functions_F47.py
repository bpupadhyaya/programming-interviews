...
Tag: fb R47/50, 636/2927, R1795/2936
"""


def exclusive_time(n: int, logs: list[str]) -> list[int]:
    helper = lambda log: (int(log[0]), log[1], int(log[2]))     # convert id and time to integer
    ...
    for (i, status, timestamp) in logs:                         # for each record
        if status == 'start':                                   # if it is start
            ...
            if s:                                               # if s is not empty
                s[-1][1] = timestamp + 1                        # update start time of previous id
    return ans
...
def main():
    ...
    print("Exclusive time: ", exclusive_time(n, logs))
