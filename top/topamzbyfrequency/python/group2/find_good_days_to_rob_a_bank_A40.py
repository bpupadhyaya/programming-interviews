...
Return a list of all days (0-indexed) that are good days to rob the bank. The order that the days are returned
in does not matter.
...
Tag: 2100/2927 , R1497/2935 , R40/50 (amz)
"""


def good_days_to_rob_bank(security: list[int], time: int) -> list[int]:
    ...
    for i in range(len(security) - time):
        if i and security[i-1] >= security[i]:
            ...
            prefix = 0
        if prefix >= time and suffix[i] >= time:
            ans.append(i)
...
def main():
    ...
    time = 2
    print(good_days_to_rob_bank(security, time))
