...
The definition of a celebrity is that all the other n - 1 people know the celebrity, but the celebrity does not know
any of them.
...
You are given a helper function bool knows(a, b) that tells you whether a knows b. Implement a function
int findCelebrity(n). There will be exactly one celebrity if they are at the party.
...
R139/145
Note: to be completed, create a graph to test. External service dependency
"""
...
def find_celebrity(n: int) -> int:
    ...
    for i in range(1, n):
        ...
        if knows(celebrity, i):
            celebrity = i
    """
    checking if the celebrity returned from the earlier loop is really a 
    celebrity by comparing it with other labelled member using knows function
    """
