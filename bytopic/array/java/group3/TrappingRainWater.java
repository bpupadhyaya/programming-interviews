...
class TrappingRainWater {
    public static void main(String[] args) {
        ...
        System.out.println("Water trapping capacity: " + trap(height));
    }
    ...
    static int trap(int[] height) {
        ...
        int totalWater = 0;
        while (left < right) { // Water could fill everythig from left to right if there is nothing in between
            if (height[left] < height[right]) {
                // If the current elevation is greater than the previous maximum, water cannot occupy that point at all.
                // However, we do know that everything from maxLeft to the current index, has been optimally filled, as
                // we have been adding water to the brim of the last maxLeft.
                if (height[left] >= maxLeft) {
                    // Found new maximum, so look to see how much water we can fill from this point on.
                    maxLeft = height[left];
                ...
                // Increment left to look at the next point.
                left++;
                ...
                if (height[right] >= maxRight) {
                    // We can say we found a new maximum and move on.
                    maxRight = height[right];
                    // If we haven't found a greater elevation, we can fill the current elevation with maxRight  - height[right] water
                } else {
